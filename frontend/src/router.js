...
const routes = [
  ...
  },
  {
    ...
    name: 'Account',
    component: () => import(`@/pages/${handleMobileView('Account')}.vue`),
    props: true,
  },
  {
    alias: '/affiliates',
    path: '/affiliates/view/:viewType?',
    name: 'Affiliates',
...
router.beforeEach(async (to, from, next) => {
  ...
  if (isLoggedIn && !users.fetched) {
    try {
      await (users.promise ?? users.fetch())
    } catch (error) {
