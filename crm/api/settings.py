...
def get_opsgate_redirect_url():
	...
	opsgate_api_url = (frappe.db.get_single_value("CRM OpsGate API Settings", "api_base_url") or "").rstrip(
		"/"
	)
	if not opsgate_api_url:
		frappe.throw(_("OpsGate API URL is not configured. Set 'API Base URL' in CRM OpsGate API Settings."))
